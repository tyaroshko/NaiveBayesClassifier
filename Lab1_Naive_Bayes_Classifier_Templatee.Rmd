...
word_freqs <- tidy_text %>%
  ...
  dplyr::mutate(word_freq = (n + 1) / (sum(n) + length(tidy_text$splitted))) %>%
...
```{r}
prob_of_first <- nrow(train%>%filter(author == "Edgar Alan Poe"))/nrow(train)
prob_of_second <- nrow(train%>%filter(author == "HP Lovecraft"))/nrow(train)
prob_of_third <- nrow(train%>%filter((author != "Edgar Alan Poe") & (author != "HP Lovecraft")))/nrow(train)
...
naiveBayes <- setRefClass("naiveBayes",
       ...
       methods = list(
                    ...
                    predict = function(message)
                    {
                      ...
                      prob_2 = 1
                      prob_3 = 1
                      rownames(word_freqs) <- word_freqs$splitted
                      for (word in splitted_message) {
                        if (not which(word_freqs$splitted == word) {
                          prob_1 = prob_1 * 1/((sum(n) + length(tidy_text$splitted)))
                        }
                        prob_1 = prob_1 * word_freqs[which(word_freqs$"splitted" == word),2]
                        # prob_2 = prob_2 * word_freqs[which(word), 2]
                        # prob_3 = prob_3 * word_freqs[word, "Mary Wollstonecraft Shelley"]
                      }
                      return(prob_1)
                    },
...
model = naiveBayes()
model$predict("She became excessively intimate with most of the ladies; and, to my profound astonishment, evinced no equivocal disposition to coquet with the men.")
```
